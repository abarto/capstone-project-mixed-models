...
type
  ...
  {** Implementacion de generador de numeros aleatorios utilizando el metodo de
      Lineal Congruencial. Ver Capitulo 7, Numerical Recipes in C, 2nd Edition.
      @abstract(Generador de numeros aleatorios basados en el metodo lineal
      congruencial.) }
  TUaGeneradorLinealCongruencial = class(TUaGeneradorNumerosAleatorios)
  private
    {** Par�metros del m�todo ran2. }
    idum: LongInt;
    idum2: LongInt;
    iy: LongInt;
    iv: array [0 .. 31] of LongInt;
  ...
  public
    ...
    constructor Create(const Semilla: LongInt); overload;
  end { TUaGeneradorLinealCongruencial };
...
// -----------------------------------------------------------------------------
// TUaGeneradorLinealCongruencial
// -----------------------------------------------------------------------------

const
  ran2_IM1 = 2147483563;
  ran2_IM2 = 2147483399;
  ran2_AM = 1.0 / ran2_IM1;
  ran2_IMM1 = ran2_IM1 - 1;
  ran2_IA1 = 40014;
  ran2_IA2 = 40692;
  ran2_IQ1 = 53668;
  ran2_IQ2 = 52774;
  ...
  ran2_IR2 = 3791;
  ran2_NTAB = 32;
  ran2_NDIV = 1 + ran2_IMM1 / ran2_NTAB;
  ran2_EPS = 1.2E-7;
  ran2_RNMX = 1.0 - ran2_EPS;

function TUaGeneradorLinealCongruencial.ran2(var idum: LongInt): Real;
var
  j: Integer;
  k: LongInt;
  temp: Real;
begin { TUaGeneradorLinealCongruencial.ran2 }
  Assert(idum <> 0, 'TUaGeneradorLinealCongruencial.ran2: idum <> 0');
  ...
  k := idum2 div ran2_IQ2;
  idum2 := ran2_IA2 * (idum2 - k * ran2_IQ2) - k * ran2_IR2;

  if idum2 < 0 then
    idum2 := idum2 + ran2_IM2;

  j := floor(iy / ran2_NDIV);
  iy := iv [j] - idum2;
  iv [j] := idum;
  ...
  temp := ran2_AM * iy;
  if temp > ran2_RNMX then
    Result := ran2_RNMX
  else
    Result := temp;
end { TUaGeneradorLinealCongruencial.ran2 };

function TUaGeneradorLinealCongruencial.GetSiguiente: TUaReal;
begin { TUaGeneradorLinealCongruencial.GetSiguiente }
  Result := ran2(idum);
end { TUaGeneradorLinealCongruencial.GetSiguiente };

constructor TUaGeneradorLinealCongruencial.Create;
begin { TUaGeneradorLinealCongruencial.Create }
  Create(-1);
end { TUaGeneradorLinealCongruencial.Create };

constructor TUaGeneradorLinealCongruencial.Create(const Semilla: LongInt);
begin { TUaGeneradorLinealCongruencial.Create }
  idum2 := 123456789;
  iy := 0;
...
begin { TUaVariableDistribucionNormal.GetSiguiente }
  ...
  Result := SiguienteMetodoBoxMuller;
