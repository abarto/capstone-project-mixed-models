...
mixto sobre el cual se desea realizar un analisis de varianza.
}
...
type
  ...
  private
    {** Datos sobre observaciones a ajustar. Estos datos incluyen informacion
    ...
    {** Indica si el modelo posee una variable de pesos. }
    FTienePesos: Boolean;
    ...
    {** Variable que contiene los pesos del modelo. }
    FVariablePeso: String;
    ...
    {** Valida las opciones del modelo.
        @param(Bitacora Registro de posibles errores y advertencias producidas
        durante el proceso de validacion.)
        @returns(Devuelve true si las opciones del modelo son validas o @false
        en caso contrario.) }
  ...
  public
    ...
    {** Constructor.
        @param(AEtiqueta Etiqueta descriptiva del modelo.)
    ...
    {** Valida la configuracion del modelo mixto en si y en relacion a la fuente
        de datos. Si se produjesen errores de validacion, los mismos pueden ser
        son ingresados a la Bitacora. Eso posible que el modelo sea valido pero
        que aun asi se produzcan errores, estos deben interpretarse como
        advertencias.
        @param(Bitacora Registro de posibles errores y advertencias producidas
        durante el proceso de validacion.)
    ...
    {** Indica si el modelo tiene pesos. }
    property TienePesos: Boolean read FTienePesos;
    ...
    {** Variable peso. }
    property VariablePeso: String read FVariablePeso write FVariablePeso;
  end;
...
begin { TConfiguracionModelo.ValidarDatos }
  ...
  { TODO: Completar reglas de validacion de relacion entre modelo y datos. }
...
begin { TConfiguracionModelo.ValidarEfectosAleatorios }
  ...
  // Definicion de efecto aleatorio global invalida o inconsistente
  I := 0;
  Listo := false;
  ...
  begin
    ...
    begin
      Bitacora.Add('Definici�n de efecto aleatorio global inv�lida o inconsistente: #' + IntToStr(I) + ' = ' + FEfectosAleatorios.Efectos [I].Texto + ' (' + Strings.CommaText + ').');
      Listo := true;
      Result := false;
    end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              Bitacora.Add('Opci�n inv�lida o inconsistente en efectos aleatorios: Cota inferior mayor que cota superior en el parametro #' + IntToStr(J) + '.');
              Listo := true;
              Result := false;
            end
            ...
            begin
              ...
              begin
                Bitacora.Add('Opci�n inv�lida o inconsistente en efectos aleatorios: Valor de paso de grilla de busqueda mayor que el rango permitido por las cotas en el parametro #' + IntToStr(J) + '.');
                Listo := true;
                Result := false;
              end;
            end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        for K := 0 to ListaInformacionParametroIndice.Count - 1 do
        begin
          ...
          begin
            ...
            begin
              if CotaInferior > CotaSuperior then
              begin
                Bitacora.Add('Opci�n inv�lida o inconsistente en efectos aleatorios: Cota inferior mayor que cota superior en el parametro #' + IntToStr(K) + '.');
                Listo := true;
                Result := false;
              end
              else if TienePaso then
              begin
                if Paso > Abs(CotaSuperior - CotaInferior) then
                begin
                  Bitacora.Add('Opci�n inv�lida o inconsistente en efectos aleatorios: Valor de paso de grilla de busqueda mayor que el rango permitido por las cotas en el parametro #' + IntToStr(K) + '.');
                  Listo := true;
                  Result := false;
                end;
              end;
...
begin { TConfiguracionModelo.ValidarOpciones }
  ...
  // Recorrer lista de opciones mientras sea necesario
  while (I < FOpciones.Count) and (not Listo) do
  begin
    Opcion := FOpciones [I];

    if Opcion.Nombre = 'peso' then
    begin
      // Validar propiedades de la opcion peso
      S := (Opcion as TOpcionParametroString).Parametro;

      // La variable de peso no corresponde a ninguna definicion de columna
      I := FDatos.Columnas.IndexOf(S);
      if I <> -1 then
      begin
        // Tipo incorrecto de columna asociada a la variable de peso
        if not ((FDatos.Columnas [I] is TColumnaDatosReales) or (FDatos.Columnas [I] is TColumnaDatosEnteros)) then
        begin
          Bitacora.Add('Tipo incorrecto de columna asociada a la variable de peso (' + S + '): ' + FDatos.Columnas [I].Texto + '.');
          Result := false;
          Listo := true;
        end
        else
        begin
          // Valor negativo en variable de pesos
          J := FDatos.Valores.Bajo;
          while (J <= FDatos.Valores.Alto) and (not Listo) do
          begin
            // Verificar que el peso no sea negativo
            R := StrToFloat(FDatos.Valores [J, I]);
            if R < 0 then
            begin
              Bitacora.Add('Valor negativo de variable de peso (' + S + '): ' + FloatToStr(R) + '.');
              Result := false;
              Listo := true;
            end;
      ...
      begin
        Bitacora.Add('La variable de peso no corresponde a ninguna definici�n de columnas (' + FVariableDependiente + ').');
        Result := false;
        Listo := false;
      end;
    end;
...
begin { TConfiguracionModelo.ValidarVariableDependiente }
  ...
  // La variable dependiente no corresponde a ninguna definicion de columna
  I := FDatos.Columnas.IndexOf(FVariableDependiente);
  if I <> -1 then
  begin
    // Tipo incorrecto de columna asociada a la variable dependiente.
    if not ((FDatos.Columnas [I] is TColumnaDatosReales) or (FDatos.Columnas [I] is TColumnaDatosEnteros)) then
    begin
      Bitacora.Add('Tipo incorrecto de columna asociada a la variable dependiente (' + FVariableDependiente + '): ' + FDatos.Columnas [I].Texto + '.');
      Result := false;
    end;
  end
  else
  begin
    Bitacora.Add('La variable dependiente no corresponde a ninguna definici�n de columnas (' + FVariableDependiente + ').');
    Result := false;
  end;
...
begin { TConfiguracionModelo.Validar }
  ...
  // Validar combinaciones de componentes
...
begin { TConfiguracionModelo.Inicializar }
  ...
  Assert(Assigned(FOpciones), 'TConfiguracionModelo.Inicializar: Assigned(FOpciones)');
  ...
  try
    ...
    // Inicializar opciones del modelo

    // Verificar la existencia de pesos
    I := FOpciones.IndexOf('peso');
    if I <> -1 then
    begin
      FTienePesos := true;
      FVariablePeso := (FOpciones [I] as TOpcionParametroString).Parametro;
    end;
